...
from spendingtracker import bcrypt, create_app, db
...
def create_default_category_set(curr_user_id):
    ...
    with create_app().app_context():
        ...
        subcat_sorted = [
            subcategories[i * 3 : (i + 1) * 3]
            for i in range((len(subcategories) + 3 - 1) // 3)
        ]

        for index, cat in enumerate(main_cats):
            new_cat = Category.create_category(
                name=cat, parent=Category.query.get(1), curr_user_id=curr_user.id
            )
            categories.append(new_cat)
            for subcat in subcat_sorted[index]:
                new_subcatcat = Category.create_category(
                    name=subcat,
                    parent=Category.query.filter_by(name=cat).first(),
...
def create_default_shopping(curr_user_id):
    ...
    with create_app().app_context():
        ...
        if not curr_user:
            ...
            return None
        category_select = [
            cat.id for cat in Category.users_all_sub_categories(curr_user=curr_user)
        ]
...
def init_app(app):
    ...
    for command in [
        reset_db,
        create_test_user,
        create_default_category_set,
        create_default_shopping,
    ]:

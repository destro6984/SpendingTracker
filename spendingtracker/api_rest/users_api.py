...
from marshmallow import fields, INCLUDE, pre_load, ValidationError
...
def register():
    ...
    if User.find_by_email(data.get('email')) or User.find_by_username(data.get('username')):
        return jsonify({"message": f"User already exists : {data.get('email'), data.get('username')}"})
    try:

...
from spendingtracker import bcrypt, db, login_manager
...
class User(db.Model, UserMixin):
    ...
    bought_products = db.relationship(
        "ProductPurchased", backref="purchased_by", cascade="all, delete-orphan"
    )
...
class ProductPurchased(db.Model):
    ...
    buy_date = db.Column(db.DateTime, nullable=False, default=datetime.today())
    ...
    def all_user_products_by_period(cls, period):
        ...
        product_result = {
            ...
            "today": cls.query.filter_by(user_id=current_user.id)
            .filter(ProductPurchased.buy_date == datetime.today().date())
            .all(),
            "7days": cls.query.filter_by(user_id=current_user.id)
            .filter(
                ProductPurchased.buy_date >= (datetime.now() - timedelta(days=7)).date()
            )
            .all(),
            "month": cls.query.filter_by(user_id=current_user.id)
            .filter(
                db.func.extract("month", ProductPurchased.buy_date)
                == datetime.now().strftime("%m")
            )
            .all(),
            "year": cls.query.filter_by(user_id=current_user.id)
            .filter(
                db.func.extract("year", ProductPurchased.buy_date)
    ...
    def sumprice_of_product_each_subcat(cls):
        sum_of_products = (
            cls.query.with_entities(
                Category.name, db.func.sum(cls.price).label("total")
            )
            .filter(cls.user_id == current_user.id)
            .outerjoin(Category, cls.category_id == Category.id)
    ...
    def sumprice_of_product_each_maincat(cls):
        ...
        period = {
            "month": db.func.extract("month", cls.buy_date)
            == datetime.now().strftime("%m"),
            "year": db.func.extract("year", cls.buy_date)
            == datetime.now().strftime("%Y"),
        }
        sum_of_products = (
            cls.query.with_entities(cat_parent.name, db.func.sum(cls.price))
            .outerjoin(Category, cls.category_id == Category.id)
            .outerjoin(cat_parent, Category.category_parent_id == cat_parent.id)
            .filter(period["month"])
            .filter(cls.user_id == current_user.id)
            .group_by(Category.category_parent_id, cat_parent.name)
            .all()
        )
        return sum_of_products
...
class Category(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    purchasedproducts = db.relationship(
        "ProductPurchased",
        backref="purchase_cat",
        lazy="subquery",
    ...
    def is_in_user_cat_set(cls, name):
        return (
            ...
            .filter(cls.users.any(id=current_user.id))
            .first()
        )
    ...
    def users_subcategories(cls, cat_name, curr_user=None):
        ...
        user_subcategories = (
            db.session.query(cls.name)
            .join(cls.parent.of_type(cat_parent))
            .filter(cat_parent.name == cat_name)
            .filter(cls.users.any(id=curr_user.id))
            .filter(cat_parent.users.any(id=curr_user.id))
            .all()
        )
    ...
    def users_main_categories(cls, curr_user=None):
        main_categories = (
            cls.query.join(cls.users)
            .filter(User.id == curr_user.id)
            .filter(cls.category_parent_id == 1)
            .all()
        )
